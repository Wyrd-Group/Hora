...
import { toAPIToolSchemas, getAvailableTools, getAllTools, registerDynamicTool } from '../data/athenaTools';
    ...
    switch (toolName) {
      ...
      case 'acquire_node': {
        ...
        if (node.lat && node.lon) {
          ...
          setTimeout(() => useEmpireStore.getState().flyToNode(node.lat, node.lon, 8, nodeId), 300);
        }
      ...
      case 'publish_feature': {
        ...
        try {
          const resp = await apiFetch('/api/v1/community-features', {
            method: 'POST',
            body: JSON.stringify({
              name: feature.name,
      ...
      case 'browse_features': {
        try {
          ...
          const resp = await apiFetch(`/api/v1/community-features?${params}`);
          if (!resp.ok) throw new Error(`API ${resp.status}`);
          const data = await resp.json();
...
export const useAthenaStore = create<AthenaState>()(
  persist(
    (set, get) => ({
      ...
      approveProposal: async (optionId, navCallbacks) => {
        ...
        for (const tc of toolCalls) {
          ...
          set(s => ({
            actionQueue: [
              ...s.actionQueue,
              { toolName: tc.name, args: tc.arguments, status: (result.startsWith('ERROR') ? 'failed' : 'done') as 'failed' | 'done', result, timestamp: Date.now() },
            ].slice(-50),
          }));
          results.push(result);
        }
      ...
      resolveToolCalls: async (option, proposal, _navCallbacks) => {
...
Return ONLY valid JSON. No explanation text.`;
      ...
      executeTool: async (toolName, args, navCallbacks) => {
        ...
        set(s => ({
          actionQueue: [
            ...s.actionQueue,
            { toolName, args, status: (result.startsWith('ERROR') ? 'failed' : 'done') as 'failed' | 'done', result, timestamp: Date.now() },
          ].slice(-50),
        }));
        return result;
      },
      ...
      executeToolCalls: async (calls, navCallbacks) => {
        ...
        for (const call of calls) {
          ...
          set(s => ({
            actionQueue: [
              ...s.actionQueue,
              { toolName: call.name, args: call.arguments, status: (result.startsWith('ERROR') ? 'failed' : 'done') as 'failed' | 'done', result, timestamp: Date.now() },
            ].slice(-50),
          }));
          results.push(result);
        }
      ...
      sendMessage: async (content, navCallbacks) => {
        ...
        try {
          ...
          const res = await apiFetch(ATHENA_TOOLS_URL, {
            method: 'POST',
            body: JSON.stringify({
              system,
          ...
          if (!res.ok) {
            ...
            const chatRes = await apiFetch(ATHENA_CHAT_URL, {
              method: 'POST',
              body: JSON.stringify({
                system,
                messages: apiMessages,
          ...
          if (data.tool_calls && data.tool_calls.length > 0) {
            ...
            const results = await get().executeToolCalls(toolCalls, navCallbacks);
            ...
            try {
              const summaryRes = await apiFetch(ATHENA_CHAT_URL, {
                method: 'POST',
                body: JSON.stringify({
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state: AthenaState | undefined) => {
        // Re-register dynamic tools into the runtime registry on load
        if (state?.dynamicToolDefs?.length) {
          state.dynamicToolDefs.forEach((t: AthenaTool) => registerDynamicTool(t));
        }
      },
    }
  )
