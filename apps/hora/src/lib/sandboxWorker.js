...
for (const name of BLOCKED) {
  try { delete self[name]; } catch { /* frozen */ }
  try { Object.defineProperty(self, name, { value: undefined, writable: false, configurable: false }); } catch { /* already frozen */ }
}

// Block prototype chain escapes (e.g. this.constructor.constructor('return fetch')())
try {
...
self.onmessage = function (e) {
  ...
  try {
    // Execute the code in a scoped proxy that intercepts all global access.
    // Only 'game' is exposed; everything else resolves to undefined.
    const scopeHandler = {
      has: () => true, // Trap all variable lookups
      get: (target, key) => {
    ...
    const scope = new Proxy(Object.create(null), scopeHandler);
    const fn = new Function('game', `"use strict";\n${code}`);
    const returnValue = fn.call(scope, game);
