...
let content = fs.readFileSync('apps/hora/src/data/shoppingData.ts', 'utf-8');
let idx = 1;
...
fs.writeFileSync('apps/hora/src/data/shoppingData.ts', content);

// Bump local storage version to force state rehydration!
let storeContent = fs.readFileSync('apps/hora/src/store/empireStore.ts', 'utf-8');
storeContent = storeContent.replace("name: 'quantico-empire-storage-v3'", "name: 'quantico-empire-storage-v4'");
fs.writeFileSync('apps/hora/src/store/empireStore.ts', storeContent);
